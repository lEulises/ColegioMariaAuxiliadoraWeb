...
// **IMPORTANTE:** Aquí DEBES implementar una verificación de autenticación
// para asegurarte de que solo el personal autorizado pueda ver esta página.
// Por ejemplo:
/*
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true || $_SESSION['user_role'] !== 'admin') {
    header("Location: login.php"); // Redirigir a una página de login
    exit;
}
*/
...
$filter_grado = $_GET['grado'] ?? ''; // Nuevo filtro
...
if (!empty($filter_grado)) {
    // Aquí puedes añadir validación extra para $filter_grado si quieres
...
<head>
    ...
    <title>Administrar Solicitudes de Cupo - Colegio María Auxiliadora</title>
        ...
        <form action="ver_solicitudes.php" method="GET" class="filter-sort-form">
            ...
            <div class="form-group">
                ...
                <select id="estado" name="estado">
                    ...
                    <option value="en_revision" <?php echo ($filter_estado == 'en_revision' ? 'selected' : ''); ?>>En Revisión</option>
                    <option value="aprobada" <?php echo ($filter_estado == 'aprobada' ? 'selected' : ''); ?>>Aprobada</option>
                    <option value="rechazada" <?php echo ($filter_estado == 'rechazada' ? 'selected' : ''); ?>>Rechazada</option>
                    <option value="inscrito" <?php echo ($filter_estado == 'inscrito' ? 'selected' : ''); ?>>Inscrito</option>
                </select>
            </div>
            <div class="form-group">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td data-label="Madre">
                                ...
                                <?php echo htmlspecialchars($solicitud['madre_cedula']); ?><br>
                                <a href="mailto:<?php echo htmlspecialchars($solicitud['madre_email']); ?>"><?php echo htmlspecialchars($solicitud['madre_email']); ?></a><br>
                                <small>Telf: <?php echo htmlspecialchars($solicitud['madre_telefono_movil']); ?></small><br>
                                <small>Dir: <?php echo htmlspecialchars(substr($solicitud['madre_direccion'], 0, 50)) . '...'; ?></small>
                            </td>
                            <td data-label="Padre">
                                <?php echo !empty($solicitud['padre_nombre_completo']) ? htmlspecialchars($solicitud['padre_nombre_completo']) : 'N/A'; ?><br>
                                <?php echo !empty($solicitud['padre_cedula']) ? htmlspecialchars($solicitud['padre_cedula']) : ''; ?>
                                <?php if (!empty($solicitud['padre_telefono_movil'])): ?><br><small>Telf: <?php echo htmlspecialchars($solicitud['padre_telefono_movil']); ?></small><?php endif; ?>
                            </td>
                            ...
                            <td data-label="Cómo se Enteró">
                                ...
                                <?php if ($solicitud['como_se_entero'] == 'Otro' && !empty($solicitud['como_se_entero_otro'])): ?>
                                    <br><small>(<?php echo htmlspecialchars($solicitud['como_se_entero_otro']); ?>)</small>
                                <?php endif; ?>
                            </td>
                            <td data-label="Miembro Inst.">
                                <?php echo htmlspecialchars($solicitud['conoce_miembro_institucion']); ?>
                                <?php if ($solicitud['conoce_miembro_institucion'] == 'SI' && !empty($solicitud['miembro_institucion_info'])): ?>
                                    <br><small>(<?php echo htmlspecialchars(substr($solicitud['miembro_institucion_info'], 0, 50)) . '...'; ?>)</small>
                                <?php endif; ?>
                            </td>
                            <td data-label="Estado"><span class="status-badge <?php echo htmlspecialchars($solicitud['estado_solicitud']); ?>"><?php echo htmlspecialchars($solicitud['estado_solicitud']); ?></span></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
