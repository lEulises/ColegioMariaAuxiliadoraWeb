    ...
    if ($noticia_id) {
        if ($_POST['action'] == 'delete') {
            ...
            if ($stmt && $stmt->bind_param("i", $noticia_id) && $stmt->execute()) {
                ...
                $message = "Error al eliminar la noticia: " . ($stmt ? $stmt->error : $conn->error);
                $message_type = 'error';
            }
            if ($stmt) $stmt->close();
            ...
            if ($stmt && $stmt->bind_param("ii", $new_status, $noticia_id) && $stmt->execute()) {
                $message = "Estado de la noticia actualizado con éxito.";
                $message_type = 'success';
            } else {
                $message = "Error al actualizar el estado: " . ($stmt ? $stmt->error : $conn->error);
                $message_type = 'error';
            }
            if ($stmt) $stmt->close();
        }
    } else {
        $message = "ID de noticia inválido para la acción.";
        $message_type = 'error';
    }
}
